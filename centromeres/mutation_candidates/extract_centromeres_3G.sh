#!/bin/bash
#SBATCH --nodes=1 
#SBATCH --job-name="extract_centromeres.20251102"
#SBATCH --cpus-per-task=64
#SBATCH --mem=64G
#SBATCH --partition=medium
#SBATCH --output=extract_centromeres.20251102.%j.log
...
for f in chr*.fa; do
  ...
  # count headers (= number of sequences) and enforce multiple of 3
  n=$(grep -c '^>' "$f")
  if (( n % 3 )); then
    echo "ERROR: $f has $n sequences (not a multiple of 3)" >&2
  fi

  k=$(( n / 3 ))  # sequences per role

  # Stream through once: first k -> grandparent, next k -> mother, last k -> granddaughter
  awk -v base="$base" -v k="$k" '
    ...
    function pick_out(i,   role, idx, suf) {
      ...
      else if (i <= 2*k) { role="mother";       idx=i-k }
      else               { role="granddaughter";idx=i-2*k }
      suf = (k>1) ? "_" idx : ""
