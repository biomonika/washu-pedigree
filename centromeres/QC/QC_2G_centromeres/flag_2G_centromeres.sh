#!/bin/bash
#SBATCH --nodes=1
#SBATCH --job-name="flag_2G_centromeres.20260608"
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --partition=medium
#SBATCH --output=flag_2G_centromeres.20260608.%j.log
...
# analyze centromeres transmitted from the grandparents to the mother, so 2G
# centromeres passed to the granddaughter are 3G, thus not included here
SAMPLES="PAN010 PAN011 PAN027"

# prepare the files
cat transmissions.2G.txt | tail -n +2 | sed 's/\t/\n/g' > transmitted_centromeres.2G.txt
...
for sample in $SAMPLES; do
    ...
    grep -F -f transmitted_centromeres.2G.txt \
        ${sample}.flagger_hifi.problems_in_centromeres.txt \
        | sort -V \
