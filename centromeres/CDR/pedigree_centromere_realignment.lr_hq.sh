...
for i in "${!BAMS[@]}"; do
    ...
    minimap2 -ax lr:hq -I 16G --eqx --cs -Y -L -y -p 0.5 -k 17 -t "$THREADS" "$REF" "$TMP_FQ1" | \
    ...
    for CHROM in "${CHROMS[@]}"; do
        for j in "${!HAPLOTYPES[@]}"; do
            ...
            #######################################################
            minimap2 -ax lr:hq -I 16G --eqx --cs -Y -L -y -p 0.5 -k 17 -t "$THREADS" "$PAN027_REF" "$TMP_FQ2" | \
                samtools view \
                    -@ "$THREADS" \
                    -bh \
                    -q "$MIN_MAPQ" \
